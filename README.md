...
You may have a character encoding problem for AgeDB dataset, if this is the case, execute the `solve_caracter_encoding_img_names()` method that is in the file `dataset.py` in `AgeDB` folder (This file replace some characters in image names).
